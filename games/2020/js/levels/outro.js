...
export default {
    ...
    start: function() {
        initSpeechBubble()
        addSpeech(['WOW...'], 1000)
        Properties.scene.time.delayedCall(1500, () => addSpeech(['THAT WAS', 'HARD'], 1300))
        // Properties.scene.time.delayedCall(4500, () => addSpeech(['WELL...'], 1500))
        Properties.scene.time.delayedCall(3500, () => addSpeech(["I'M SURE 2021 WILL", 'BE MUCH BETTER!'], 2500))
        // Start showing 2021
...
function initSpeechBubble() {
    ...
    let line1 = Properties.scene.add.bitmapText(0, 0, 'dark').setOrigin(0.5)
    let line2 = Properties.scene.add.bitmapText(0, 0, 'dark').setOrigin(0.5)
...
function addSpeech(lines, delay = 1000) {
    ...
    if (maxLineLength > 12) { fontSize = 24 }
