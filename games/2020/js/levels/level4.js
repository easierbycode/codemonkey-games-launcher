...
const CLOUD_TEXTS = {
    440: ['ZOOM CALL'],
    442: ['YOU ARE', 'ON MUTE'],
    443: ['CAN YOU SEE', 'MY SCREEN?'],
    444: ['YOUR CONNECTION', 'IS BAD'],
    445: ['SORRY FOR THE', 'BACKGROUND NOISE'],
    446: ['CAN YOU', 'HEAR ME?'],
    447: ['YOU ARE', 'BREAKING UP'],
    448: ['CAN YOU', 'REPEAT?'],
    449: ['STAY SAFE'],
}
...
export default {
    ...
    showTitle: function() {
        // Show title
        fadeInOutTitle('LOCKDOWNS CLEAR NATURE', null, 3000)
    },
...
function addCloudText(cloud) {
    ...
    if (textLines.length === 1) {
        ...
        text.setOrigin(0.5).setDepth(Constants.DEPTH.important)
    } else {
        // Texts for two lines
        let offset = fontSize / 2 + 2
        ...
        text1.setOrigin(0.5).setDepth(Constants.DEPTH.important)
        text2.setOrigin(0.5).setDepth(Constants.DEPTH.important)
    }
}
