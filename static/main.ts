const coverflowEl = document.getElementById('coverflow') as HTMLElement;
const gameframe = document.getElementById('gameframe') as HTMLIFrameElement;
const zipInput = document.getElementById('zip-input') as HTMLInputElement;
const addZipBtn = document.getElementById('add-zip') as HTMLButtonElement;
const addGithubBtn = document.getElementById('add-github') as HTMLButtonElement;
const osd = document.getElementById('osd') as HTMLElement;
const osdClose = document.getElementById('osd-close') as HTMLButtonElement;
const captureThumbBtn = document.getElementById('capture-thumb') as HTMLButtonElement;
const exitGameBtn = document.getElementById('exit-game') as HTMLButtonElement;

type Game = { id: string; name: string; urlPath: string; hasThumbnail: boolean };
let games: Game[] = [];
let focusedIndex = 0;
...
function wrapText(ctx: CanvasRenderingContext2D, text: string, _x: number, _y: number, maxWidth: number, _lineHeight: number): string[] {
...
function renderCoverflow() {
  ...
  games.forEach((g, i) => {
    const card = el('div', 'card dim') as HTMLDivElement;
    ...
    (card as HTMLDivElement).style.backgroundImage = `url(${bg})`;
...
function focusIndex(i: number, open: boolean) {
...
function openGame(game: Game) {
  ...
  const req = root.requestFullscreen?.bind(root)
    || (document as any).body?.webkitRequestFullscreen?.bind((document as any).body)
...
function isOSDKey(e: KeyboardEvent) {
  const k = e.key;
  // Support ` and ~ and Backquote code (keyCode 192)
  return k === '`' || k === '~' || (e as any).code === 'Backquote' || (e as any).keyCode === 192;
}
...
window.addEventListener('keydown', globalKeyHandler, { capture: true } as any);
...
function toggleOSD(show: boolean) {
...
async function captureIframeCanvas(iframe: HTMLIFrameElement): Promise<string | null> {
...
function bindIframeKeys() {
  try {
    ...
    w.addEventListener('keydown', handler, { capture: true } as any);
  } catch {
